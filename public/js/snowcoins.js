...
snowUI.receive = React.createClass({displayName: 'receive',
	...
	snowUI.methods.loaderStop();
	    return (
		React.DOM.div({className: "reactfade", id: "maindiv"}, "  Receive ")		
...
snowUI.settings = React.createClass({displayName: 'settings',
	...
	render: function() {
	    ...
	    snowUI.methods.loaderStop();
	    return (
		React.DOM.div({className: "reactfade", id: "maindiv"}, "  Settings ")				
...
snowUI.inq = React.createClass({displayName: 'inq',
	...
	render: function() {
		...
		snowUI.methods.loaderStop();
		return (
			React.DOM.div({className: "reactfade", id: "maindiv"}, "  Inqueue ")			
		...
		if(nextProps.config.wallet && nextProps.config.wallet !== 'new') {
			...
			snowUI.methods.ajax.call("/api/snowcoins/local/wallet",{ wallet:nextProps.config.wallet,moon:'status' },function(resp) {
				...
				if(resp.success === true)
				{
					...
					snowUI.methods.changelock(resp.data.unlocked_until || 0);
