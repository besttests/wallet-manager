$(function() {  
	...
	var snowtext = snowlanguages.default;
	...
	function startboneviews() {
		...
		bone.view.Searchbar = bone.view('#walletbar .search', {
			...
			find: function(event) {
				...
				bone.router.navigate('/d3c/find/' + el.buttons.findme.find('div').attr('snow-link') + '/' +searchbar.val(), {trigger: true});
			...
			searchfor: function(event) {
				...
				if(bar!=='') bone.router.navigate('/d3c/find/'+el.buttons.findme.find('div').attr('snow-link') + '/' +bar, {trigger: true});
			}
		});
		/* create menu */
		bone.view.Create = bone.view('#menubar .create .body', {
			...
			ledger: function(event) {
				bone.router.navigate('/d3c/create/ledger', {trigger: true});
			},
			item: function(event) {
				bone.router.navigate('/d3c/create/item', {trigger: true});
			},
			tx: function(event) {
				bone.router.navigate('/d3c/create/tx', {trigger: true});
		...
		bone.view.View = bone.view('#menubar .view .body', {
			...
			ledger: function(event) {
				bone.router.navigate('/d3c/ledger', {trigger: true});
			},
			item: function(data) {
				bone.router.navigate('/d3c/view/item', {trigger: true});
			},
			tx: function(data) {
				bone.router.navigate('/d3c/view/tx', {trigger: true});
			}
		});
		...
		bone.view.Connect = bone.view('#menubar .connect .body', {
			...
			client: function(event) {
				bone.router.navigate('/d3c/connect/client', {trigger: true});
			},
			customer: function(data) {
				bone.router.navigate('/d3c/connect/customer', {trigger: true});
			}
		});
		...
		bone.view.Modal = bone.view('#confirm-modal ', {
			...
			cancelledger: function(e) {
				...
				bone.router.navigate('/d3c/ledger/' + key + '/cancelled/', {replace: true});
			...
			deleteledger: function(e) {
				...
				bone.router.navigate('/d3c/ledger/' + key + '/deleted/', {replace: true});
				bone.view.Modal.close();
			},
			close: function(e) {
				this.$el.modal('hide');
		...
		bone.view.Manage = bone.view('#manage .content ', {
			...
			gotxmanual: function(event) {
				bone.router.navigate('/d3c/create/tx/'+el.inputs.ledgerkey.val(), {trigger: true});
			},
	...
	function startboneroutes() {
		...
		bone.router({
			routes: {
				"d3c/refresh":         "wait",   
				"d3c/create/:what/:key":         "create", 
				"d3c/create/:what":         "create",  
				"d3c/find/:what/:query":          "search",   
				"d3c/ledger/:lid":    	"ledger",
				"d3c/ledger":    	"ledger",
				"d3c/view/:what":    		"view",     
				"d3c/connect/:who":   		"connect",    
				"d3c/reset":   		"reset",
				"d3c/login":   		"login",   
			},
			middleware: [
		...
		set listledgers (data) {
			...
			if(data && data.length>0 && data instanceof Array) {
				data.forEach(function(val) {
					...
					tmp.push('<tr>');
					tmp.push('<td class="">');
					tmp.push('<td><a onclick="bone.router.navigate(\'/d3c/ledger/'+val.ledgerID+'\', {trigger: true});">'+val.ledgerID+'</a>');
			...
			if(el.ledgerentries._order) {
				...
				.autocomplete({ 
					...
					select: function( event, ui ) {
						...
						bone.router.navigate('/d3c/ledger/'+ui.item.value, {trigger: true});
		...
		ledger: {
			...
			create: function(resp) { 
				...
				if(resp.data.success==true) {
					...
					bone.router.navigate('/d3c/ledger/'+resp.data.data.ledgerID, {trigger: true});
	...
	function authmaster(uri,options) {
		...
		/* start routes */
		startboneroutes();
		bone.router.navigate('/d3c/login/', {trigger: true});
