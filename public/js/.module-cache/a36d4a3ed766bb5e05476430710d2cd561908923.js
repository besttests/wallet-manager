...
var App = React.createClass({displayName: 'App',
	...
	render: function() {
		return (
			React.DOM.div(null, 
			React.DOM.div({id: "walletbar"}, 
			  React.DOM.div({className: "wallet"}, 
				React.DOM.div({className: "btn-group"}, 
					Button({bsStyle: "link", 'data-toggle': "dropdown", className: "dropdown-toggle"}, snowtext.menu.menu.name), 
					React.DOM.ul({className: "dropdown-menu", role: "menu"}, 
						React.DOM.li({className: "nav-item-add"}, " ", React.DOM.link({to: ""}, snowtext.menu.plus.name)), 					
						React.DOM.li({className: "nav-item-home"}, " ", React.DOM.link({to: "overview"}, snowtext.menu.list.name)), 
						React.DOM.li({className: "nav-item-receive"}, React.DOM.link({to: "receive"}, snowtext.menu.receive.name)), 
						React.DOM.li({className: "divider"}), 
						React.DOM.li({className: "nav-item-snowcat"}, React.DOM.link({to: ""}, locals.user.username)), 
						React.DOM.li({className: "divider"}), 
						React.DOM.li(null, 
							React.DOM.div(null, 
...
var routes = (
	Routes({location: "history"}, 
		Route({name: "snowcoins", path: "/react", handler: App}, 
			DefaultRoute({handler: SnowpiMan})
		), 
		NotFoundRoute({handler: App})
	)
);

React.renderComponent(App(null), document.getElementById('snowcoins-react'));
