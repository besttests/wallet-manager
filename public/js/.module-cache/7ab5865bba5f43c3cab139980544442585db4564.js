		...
		return this.transferPropsTo(
			Modal({title: this.props.title, onRequestHide: this.handleToggle}, 
				...
				React.DOM.div({className: "modal-footer"}, 
					 this.props.footer ? '<div className="clearfix">' + this.props.footer + '</div>' : '', 
...
var UI = React.createClass({displayName: 'UI',
	getInitialState: function() {
		...
		return {
			...
			lockstatus:2,
			modals: {
				unlockWallet: {open:false},
				encryptWallet: {open:false},
			}
		};
	},
	...
	encryptWallet: function() {
		...
		modals.encryptWallet.show = true;
		this.setState({modals:modals});
	},
	unlockWallet: function() {
		var modals = this.state.modals;
		modals.unlockWallet.show = true;
		this.setState({modals:modals});
	...
	render: function() {
		...
		if(this.state.modals.unlockWallet) {
			var uButtons = Button(null, "Unlock")
			unlockWallet = (addModal({title: "Unlock" + this.state.wally.name, buttons: uButtons, footer: "message for footer"}, "This is the body") )
		}
		if(this.state.modals.encryptWallet) {
			var eButtons = Button(null, "Encrypt")
			encryptWallet = (addModal({title: "Unlock" + this.state.wally.name, buttons: eButtons, footer: "message for footer"}, "This is the body") )
		}
		if(this.state.modals.passWallet) {
			var pButtons = Button(null, "Hello!")
			passWallet = (addModal({title: "Unlock" + this.state.wally.name, buttons: pButtons, footer: "message for footer"}, "This is the body") )
		}
