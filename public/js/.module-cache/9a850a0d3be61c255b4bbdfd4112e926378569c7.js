...
snowUI.dashboard = React.createClass({displayName: 'dashboard',
	getInitialState: function() {
		...
		return ({data:false,mounted:false});
	...
	getData: function (cb) {
		...
		snowUI.methods.ajax.call(url,data,function(resp) {
			console.log(resp)
			this.setState({data:resp.data,mounted:true});
			cb(true)
		}.bind(this))
	},
	...
	render: function() {
		...
		if(this.state.mounted) {
			...
			return (
			React.DOM.div({class: "snow-dashboard"}, 
				React.DOM.div({className: "snow-block snow-balance"}, 
					...
					React.DOM.div({className: "snow-balance-body"}, 
						React.DOM.p(null, data.balance, " ", React.DOM.span({class: "coinstamp"}, this.props.config.wally.coinstamp, " "))
					)
				)
