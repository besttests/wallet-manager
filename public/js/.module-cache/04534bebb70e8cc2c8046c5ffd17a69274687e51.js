...
snowUI.wallet = React.createClass({displayName: 'wallet',
	getInitialState: function() {
		return ({
			config:this.props.config || {section:snowPath.wallet,wallet:'overview',moon:false}
		})
	},
	...
	render: function() {
	    ...
	    var renderMe = snowUI[this.state.config.moon || this.state.config.wallet]
					...
					React.DOM.div({className: "container-fluid"}, 
						...
						React.DOM.div({className: "dogeboard col-xs-11 col-md-10"}, 
							...
							React.DOM.div({className: "dogeboard-left col-xs-12 col-md-12"}, 
								...
								mycomp({methods: methods, config: this.state.config})
							)
						)
					)
				/* end snowpi-body */
