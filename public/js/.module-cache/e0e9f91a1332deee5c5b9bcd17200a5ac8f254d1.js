...
snowUI.wallet = React.createClass({displayName: 'wallet',
	...
	render: function() {
		...
		return (
			...
			React.DOM.div({className: "reactfade", id: "maindiv"}, " ", renderMe({config: this.state.config, ready: ready}), " ")
...
snowUI.dashboard = React.createClass({displayName: 'dashboard',
	...
	componentWillUpdate: function () {
		var _this = this
		//if(!this.state.data)this.getData(function(resp){ _this.setState({data:resp.data,mounted:true}) })
		//snowlog.log('dashboard will mount',this.props)
	},
	componentDidMount: function() {
		//var _this = this
		//this.getData(function(resp){ _this.setState({data:resp.data,mounted:true}) })
	},
	componentWillUnMount: function() {
