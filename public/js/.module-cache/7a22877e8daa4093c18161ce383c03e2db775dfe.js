		...
		return this.transferPropsTo(
			React.DOM.div({className: "list"}, 
				...
				React.DOM.select({onChange: this.route, id: "walletselect"}, 
					...
					React.DOM.option({value: snowPath.root + snowPath.wallet}, "list wallets"), 
					React.DOM.option({value: snowPath.root + snowPath.receive}, "receive coins"), 
					React.DOM.option({value: snowPath.root + snowPath.inqueue}, "inquisive queue")
				)
			)
		);
	}
