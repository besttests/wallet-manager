...
snowUI.leftMenu = React.createClass({displayName: 'leftMenu',
	...
	render: function() {
		var menu = this.state.section === snowPath.wallet ? snowUI.walletMenu : (this.state.section === snowPath.receive || this.state.section === snowPath.settings) ? snowUI.receiveMenu : snowUI.defaultMenu
	    console.log('main menu component')
	    return (
		React.DOM.div(null, React.DOM.menu({config: this.state.config}), " ")			
...
snowUI.receiveMenu = React.createClass({displayName: 'receiveMenu',
	...
	render: function() {
	    ...
	    console.log('rewceive menu component')
	    return (
		React.DOM.div(null, 
		"receivers / settings"
		)			
