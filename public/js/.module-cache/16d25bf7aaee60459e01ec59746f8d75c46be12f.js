			...
			if(wally.coinapi==='rpc')
			{
				snowUI.methods.ajax.callwaiting("/api/snowcoins/local/wallet",{ wallet:nextProps.config.wallet,moon:'status' },function(resp) {
					...
					if(resp.success === true)
					{
						...
						_this.setState({testnet:false,ready:false})
...
snowUI.dashboard = React.createClass({displayName: 'dashboard',
	...
	componentWillReceiveProps: function (nextProps) {
		var _this = this
		//if(nextProps.ready && !this.state.mounted)this.getData(nextProps,function(resp){_this.setState({data:resp.data,mounted:true,ready:nextProps.ready}) })
		snowlog.log('dashboard will receive props',this.props)
	},
	componentWillUpdate: function () {
		var _this = this
		if(this.state.ready && !this.state.mounted)this.getData(this.state,function(resp){_this.setState({data:resp.data,mounted:true,ready:this.props.ready}) })
	...
	componentDidMount: function() {
		...
		//if(this.state.ready)this.getData(this.props,function(resp){ _this.setState({data:resp.data,mounted:true}) })
	},
	componentWillUnMount: function() {
