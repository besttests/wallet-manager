...
snowUI.wallet = React.createClass({displayName: 'wallet',
	...
	componentWillReceiveProps: function(nextProps) {
		...
		_this.setState({config:nextProps.config,connectError:false,connected:false})
	},
				...
				snowUI.methods.ajax.call("/api/snowcoins/local/wallet",{ wallet:nextProps.config.wallet,moon:'status' },function(resp) {
					...
					if(resp.success === true)
					{
						...
						if(resp.data && resp.data.testnet===true) {
							...
							_this.setState({testnet:true,ready:true,connected:true})
							
						} else {
							
							
							_this.setState({testnet:false,ready:true,connected:true})
						
						}
						//changelock(resp.data.unlocked_until || 0);
						
					} else {
						_this.setState({testnet:false,ready:false,connectError:resp.error,connected:false})
						snowmessage('error','Please try again. <br />',3000);
					}
	...
	render: function() {
		...
		if(!this.state.connected) {
			...
			return (
				...
				React.DOM.div({className: "reactfade", id: "maindiv"}, " ", snowUI.connectError({config: this.state.config, ready: this.state.ready, connected: this.state.connected, setWalletState: this.updateState, message: this.state.connectError}), " ")
			...
			return (
				...
				React.DOM.div({className: "reactfade", id: "maindiv"}, " ", renderMe({config: this.state.config, ready: this.state.ready, connected: this.state.connected, setWalletState: this.updateState}), " ")
...
snowUI.dashboard = React.createClass({displayName: 'dashboard',
	getInitialState: function() {
		...
		return {mounted:false,ready:this.props.ready,connected:this.props.connected};
	...
	componentWillReceiveProps: function (nextProps) {
		...
		if(this.props.config.wallet !== nextProps.config.wallet)
			this.getData(nextProps,function(resp){_this.setState({data:resp.data,mounted:true,ready:nextProps.ready,connected:nextProps.connected}) })
	...
	getData: function (props,cb) {
		...
		snowUI.methods.ajax.call(url,data,function(resp) {
			...
			if(resp.success === true) {
				...
				_this.props.setWalletState({connectError:true,connected:false})
			}
		})
		return false
	},
	...
	render: function() {
		...
		snowlog.log('wallet dashboard component',this.state.mounted,this.state.connected)
