...
snowUI.walletMenu = React.createClass({displayName: 'walletMenu',
	...
	render: function() {
		var testnet = this.props.config.testnet ? (React.DOM.div({id: "testnet-flash", title: "", 'data-toggle': "tooltip", 'data-placement': "right", 'data-container': "body", class: "dogemenulink", 'data-original-title': "This wallet is on the TESTNET!", style: {display:'block'}}, React.DOM.span({className: "glyphicon glyphicon-text-width"}), " TESTNET ")) : ''
...
var UI = React.createClass({displayName: 'UI',
	...
	getWallets: function () {
		//grab our initial data
			...
			$.ajax({url: "/api/snowcoins/local/change-wallet"})
				.done(function( resp,status,xhr ) {
					
					_csrf = xhr.getResponseHeader("x-snow-token");
					snowlog.log('wally',resp.wally)
	...
	componentWillReceiveProps: function(nextProps) {
		...
		if(nextProps.wallet !== undefined) {
			var newWally = this.state.config.mywallets[this.state.config.locatewallet.indexOf(nextProps.config.wallet)];
			this.setState({wally: newWally });
		}
	...
	render: function() {
		...
		var testnet = this.state.config.testnet ? 'testnet':''
		...
		return (
			React.DOM.div({id: "snowpi-body"}, 
				...
				React.DOM.div({className: "container-fluid"}, 
					React.DOM.div({id: "menuspy", className: "dogemenu col-xs-1 col-md-2 " + testnet, 'data-spy': "affix", 'data-offset-top': "0"}, 
