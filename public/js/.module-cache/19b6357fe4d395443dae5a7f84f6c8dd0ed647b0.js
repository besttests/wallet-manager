		...
		return this.transferPropsTo(
			React.DOM.div({className: "list"}, 
				...
				React.DOM.select({onChange: this.route, id: "walletselect"}, 
					...
					React.DOM.option({value: snowPath.wallet, selected: (this.props.section === snowPath.wallet.substring(1) && this.props.moon === 'overview') ? 'selected':''}, "list wallets"), 
					React.DOM.option({value: snowPath.receive, selected: (this.props.section === snowPath.receive.substring(1))  ? 'selected':''}, "receive coins"), 
					React.DOM.option({value: snowPath.inq, selected: (this.props.section === snowPath.inq.substring(1))  ? 'selected':''}, "inquisive queue")
				)
			)
		);
	}
