...
snowUI.loading = React.createClass({displayName: 'loading',
	...
	componentDidMount: function() {
		var _this = this
		if (_this.isMounted()) {
			snowUI.passthrough.loader = {}
			snowUI.passthrough.loader.start = function() {
				console.log('start loading passthrough')
				if (_this.isMounted())_this.setState({active:true});
			}
			snowUI.passthrough.loader.stop = function() {
				console.log('stop loading passthrough')
				if (_this.isMounted())_this.setState({active:false});
			}
		}
	},
	render: function() {
		...
		if(this.state.active) {
			return (React.DOM.div({key: "load", className: "loader"}, 
...
var UI = React.createClass({displayName: 'UI',
	...
	componentDidMount: function() {
		...
		.done(function( resp,status,xhr ) {
			...
			//add the loader
			React.renderComponent(snowUI.loading(null), document.getElementById('snowcoins-loader'));
		}.bind(this));
	},
	...
	loaderStart: function() {
		if(typeof snowUI.passthrough.loader === 'object') snowUI.passthrough.loader.start()
		return false
	},
	loaderStop: function() {
		if(typeof snowUI.passthrough.loader === 'object') snowUI.passthrough.loader.stop()
		return false
	},
