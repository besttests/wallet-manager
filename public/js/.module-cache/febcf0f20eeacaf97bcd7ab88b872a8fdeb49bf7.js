...
var addModal = React.createClass({displayName: 'addModal',
	mixins: [OverlayMixin],
	...
	componentWillReceiveProps: function(np) {
		if(np.open)this.setState({isModalOpen:np.open})
	},
		...
		return (
			Modal({title: this.props.title, onRequestHide: this.handleToggle}, 
				...
				React.DOM.div({className: "modal-footer"}, 
					...
					React.DOM.div({className: "col-xs-12 col-sm-6 col-md-6", style: {textAlign:'right'}}, 
						Button({bsStyle: "default", onClick: this.handleToggle}, "Cancel")
					)
...
var UI = React.createClass({displayName: 'UI',
			...
			ajax: {
				...
				send: function(type,url,data,callback) {
					if(!snowUI.methods.ajax.running) {
						snowUI.methods.ajax.forced(type,url,data,callback)
					} else {
						snowmessage('message','call in progress... ' + snowUI.methods.ajax.running,2000)
					}
				},
				post: function(url,data,callback) {
					snowUI.methods.ajax.send('POST',url,data,callback)
				},
				call: function(url,data,callback) {
					snowUI.methods.ajax.send('GET',url,data,callback)
				},
				callwaiting: function(type,url,data,callback) {
					if(!snowUI.methods.ajax.running) snowUI.methods.ajax.running = url
					snowUI.methods.ajax.send(type,url,data,callback)
				},
				running: false,
	...
	render: function() {
		...
		var lockedwallet = this.state.wallet &&  this.state.locked && !this.state.unlocked ? "pointer " : "pointer hidden"
		var openwallet = !this.state.wallet || this.state.locked ? "pointer hidden" : "pointer "
		var unlockedwallet = this.state.wallet &&  this.state.unlocked ? "pointer " : "pointer hidden"
		
		var testnet = this.state.testnet ? 'testnet':''
		snowlog.log('testnet',this.state.testnet)
		
		var encryptWallet,unlockWallet,passWallet;
		 
		var uButtons = Button(null, "Unlock")
		unlockWallet = (addModal({open: this.state.modals.unlockWallet, title: "Unlock " + this.state.wally.name, buttons: uButtons, footer: "message for footer"}, "This is the body") )
		
		var eButtons = Button(null, "Encrypt")
		encryptWallet = (addModal({open: this.state.modals.encryptWallet, title: "Encrypt " + this.state.wally.name, buttons: eButtons, footer: "message for footer"}, "This is the body") )
		
		var pButtons = Button(null, "Pass")
		passWallet = (addModal({open: this.state.modals.passWallet, title: "Change pass for " + this.state.wally.name, buttons: pButtons, footer: "message for footer"}, "This is the body") )
		...
		return (
			React.DOM.div({id: "snowpi-body"}, 
				...
				React.DOM.div({id: "walletbar", className: "affix"}, 
					...
					React.DOM.div({onClick: this.unlockWallet, className: lockedwallet, id: "wallet-lock", 'data-toggle': "tooltip", 'data-placement': "bottom", alt: "Wallet is encrypted and locked", title: "Wallet is encrypted and locked"}), 
					
					React.DOM.div({onClick: this.encryptWallet, className: openwallet, id: "wallet-unlock", snowlink: "dashboard", 'data-toggle': "tooltip", 'data-placement': "bottom", title: "You should encrypt this wallet soon."}), 
