...
var UI = React.createClass({displayName: 'UI',
	...
	componentDidMount: function() {
		if(!this.state.mounted) {
			...
			$.ajax({url: "/api/snowcoins/local/contacts/?setnonce=1"})
	...
	render: function() {
		...
		if(this.state.mountedh) {
			var mountme = function() {
				return (React.DOM.div(null, " mounting app "))
			}
		...
		}//end our mounted ui return
		//mount
		
		return ( React.DOM.div(null, " ", mountme(), " ")	)
	}
});
