...
snowUI.wallet = React.createClass({displayName: 'wallet',
	...
	componentWillReceiveProps: function(nextProps) {
		...
		_this.setState({config:nextProps.config,connectError:false})
	},
			...
			if(wally.coinapi==='rpc')
			{
				snowUI.methods.ajax.call("/api/snowcoins/local/wallet",{ wallet:nextProps.config.wallet,moon:'status' },function(resp) {
					...
					if(resp.success === true)
					{
						...
						_this.setState({testnet:false,ready:false,connectError:true})
	...
	render: function() {
		...
		if(this.state.connectError) {
			...
			return (
				...
				React.DOM.div({className: "reactfade", id: "maindiv"}, " ", snowUI.connectError({config: this.state.config, ready: this.state.ready, setWalletState: this.updateState}), " ")
...
snowUI.connectError = React.createClass({displayName: 'connectError',
	...
	render: function() {
	    ...
	    return (
		React.DOM.div(null, 
			React.DOM.div({className: "requesterror"}, React.DOM.span(null, 
				"Sorry, ", this.props.config.wally.name, " is unreachable at the moment."
			))
		)			
