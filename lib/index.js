...
Snowcoins.prototype.set = function(key,value) {
	...
	if(key.search("wallets")>-1 && key !== 'model wallets') {
		this.set(key.replace("wallets","snowcoins"),value);
	}		
	switch(key) {
		case "use snowcoins user model":
			if(value) this._options['model user'] = this._options['snowcoins user model'] || this._options['wallets user model'];
			break;
		case "use wallets user model":
			if(value) this._options['model user'] = this._options['wallets user model'] || this._options['snowcoins user model'];
			break;
		case ".link ip range":
