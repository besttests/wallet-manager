...
var Snowcoins = function() {
	...
	this._options = {
		get lists() { 
			return {
				...
				coins: keystone.list(_this.get('model coins')),
				wallets: keystone.list(_this.get('model wallets')),
				wallettx: keystone.list(_this.get('model wallet transactions')),
				contacts: keystone.list(_this.get('model contacts')),
				clients: keystone.list(_this.get('model clients')),
				rates: keystone.list(_this.get('model rates')),
				attended: keystone.list(_this.get('model attended')),
				unattended: keystone.list(_this.get('model unattended')),
				ledger: keystone.list(_this.get('model ledger')),
				
				trackers: keystone.list(_this.get('model trackers')),
				transactions: keystone.list(_this.get('model transactions')),
				items: keystone.list(_this.get('model items')),
				log: keystone.list(_this.get('model log')),
				snowmoney: keystone.list(_this.get('model snowmoney')),
			}
		},
		'.link ip range' : [],
	}
	...
	//uri paths 
	this.set('path snowcoins','snowcoins');
...
Snowcoins.prototype.linkServer = function(state,callback) {
	...
	if(typeof state !== 'function' && typeof state !== 'object') {
		if(state === 'yes' || state === 'no')state = {state:state};
